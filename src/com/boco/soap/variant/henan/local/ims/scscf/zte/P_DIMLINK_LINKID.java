...
//参数核查中横表多行中兴MGCF的show DIM LINK的,取DIM LINK中LINK ID变量的方法
  ...
  extends VariantValueInvoke
{
  ...
  public String[] getValues(INeElement arg0, IInstructionParameter arg1, Map<String, ?> data, String arg3)
  {
    ...
    if ((objNeType.toString().trim().toUpperCase().equals("SCSCF")) && 
      (objActTbName.toString().trim().toUpperCase().equals("DIMLINK")) && (paraName != null))
    {
  ...
  private void initMap(String dbFile)
  {
    ...
    String sql = "SELECT DEVICENAME,GROUP_CONCAT(LINKID) LINKID FROM ZTE_VOLTE_CSCF_DIMLINK GROUP BY DEVICENAME";
    List<Map<String, ?>> resultList = utils.getLocalData(sql, dbFile);
    for (Map<String, ?> temp : resultList)
    {
      String deviceName = temp.get("DEVICENAME").toString();
      String identifiers = temp.get("LINKID").toString();
