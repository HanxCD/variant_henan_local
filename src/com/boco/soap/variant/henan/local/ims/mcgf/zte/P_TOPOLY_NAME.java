...
//参数核查中横表多行中兴MGCF的show TOPOLY的,取TOPOLY中TPCNAME变量的方法
  ...
  extends VariantValueInvoke
{
  ...
  public String[] getValues(INeElement arg0, IInstructionParameter arg1, Map<String, ?> data, String arg3)
  {
    ...
    if ((objNeType.toString().trim().toUpperCase().equals("MGCF")) && 
      (objActTbName.toString().trim().toUpperCase().equals("TOPOLY")) && (paraName != null))
    {
  ...
  private void initMap(String dbFile)
  {
    ...
    String sql = "SELECT DEVICENAME,GROUP_CONCAT(TPCNAME) TPCNAME FROM ZTE_MGCF_TOPOLY GROUP BY DEVICENAME";
    List<Map<String, ?>> resultList = utils.getLocalData(sql, dbFile);
    for (Map<String, ?> temp : resultList)
    {
      String deviceName = temp.get("DEVICENAME").toString();
      String identifiers = temp.get("TPCNAME").toString();
