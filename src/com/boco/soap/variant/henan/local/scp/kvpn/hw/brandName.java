  ...
  extends VariantValueInvoke
{
  public String[] getValues(INeElement arg0, IInstructionParameter arg1, Map<String, ?> data, String dbFile)
  {
    ...
    if (startGt == null) {
      ...
      scpNo = getScpNo(startGt.toString().trim(), areaCode.toString().trim(), areaCity, dbFile);
    }
  ...
  private String getScpNo(String startGt, String areaCode, Object areaCity, String dbFile)
  {
    ...
    String sql = "select GT,BRAND from TCM_VPMN_SCP where AREA_CODE='" + areaCode + "'";
    ...
    if (resultList.size() != 0)
    {
      ...
      {
        result[i] = ((Map)resultList.get(i)).get("GT").toString();
        if (startGt.startsWith(result[i]))
        {
          scpNo = ((Map)resultList.get(i)).get("BRAND").toString();
          break;
        }
      }
      return scpNo;
