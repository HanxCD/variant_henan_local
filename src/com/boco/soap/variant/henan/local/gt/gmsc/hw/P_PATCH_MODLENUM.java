...
//DATE 2018-05-16  wanghao25  华为GMSS查询PATCH时查询模块号
...
public class P_PATCH_MODLENUM extends VariantValueInvoke
{
...
(objActTbName.toString().trim().toUpperCase().equals("PATCH")) && 
(paraName != null)) {
...
String sql = "SELECT DEVICENAME,GROUP_CONCAT(MODLENUM) MODLENUM FROM V_HW_GMSS_PATCH_P GROUP BY DEVICENAME";
List<Map<String, ?>> resultList = utils.getLocalData(sql, dbFile);

for (Map<String, ?> temp : resultList) {
String deviceName = temp.get("DEVICENAME").toString();
String identifiers = temp.get("MODLENUM").toString();
