 ...
 *
 */
public class HW_GMSS_CLDPREANA_PFX_COVER_CITY_MAIN extends VariantValueInvoke {
    ...
    public String[] getValues(INeElement ne, IInstructionParameter arg1, Map<String, ?> data, String arg3) {
        List<String> coverCityList = this.getCoverCity(ne);
        ...
        if (null == o) {
            ...
            for (String tempStr : coverCityList) {
    ...
    private String getCoverCity(INeElement ne, String dbFile) {
        ...
        String sql = "SELECT BUSI_CITY FROM TCM_HW_GMSS_MTB_REL WHERE DEVICENAME ='" + ne.getName() + "'";
        List<Map<String, ?>> resultList = utils.getLocalData(sql, dbFile);
        String coverCity = "";
        for (Map<String, ?> temp : resultList) {
            coverCity += "," + temp.get("BUSI_CITY");
        }
        return StringUtils.isEmpty(coverCity) ? "[NULL]" : coverCity.substring(1);
    }*/
