...
public class HW_SCPAS_NUMTYPE implements ISpliteOperate{
	...
	public List<Map<String, String>> operate(List<Map<String, String>> oldList) {
		...
		for (Map<String, String> map : oldList) {
			...
			if(start.startsWith("1")){
				if(type.equals("3")||type.equals("4")){//移动 前七位细分号段数据
...
//					String MODIFIER = map.get("MODIFIER");
					   ...
					   for(int i=0;i<=count;i++){
						   ...
						   int Startnum=StartNo+i;
							...
							newMap.put("ISNUMTYPE", map.get("ISNUMTYPE"));
							newMap.put("BUSSNUM", map.get("BUSSNUM"));
							newMap.put("ISIMS", map.get("ISIMS"));
							newMap.put("ISJOINGROP", map.get("ISJOINGROP"));
							newMap.put("INTELJOIN", map.get("INTELJOIN"));
							newMap.put("ISTIETONGUSER", map.get("ISTIETONGUSER"));
							newMap.put("ISNEWNUM", map.get("ISNEWNUM"));
